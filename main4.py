...
def countCarState(fn, max_frame, state_dict):
    ...
    # Process each frame and log movements
    for frame in range(max_frame):
        if frame in dict_input:
            logger.info(f"Frame {frame} movements:")
            for movement in dict_input[frame]:
                lane = movement['lane']
                movement_id = movement['movement']
                obj_type = movement['obj_type']
                logger.info(f"  - Lane {lane} (Movement {movement_id}): Object type {obj_type}")

        current_state = lst.copy()
        if frame in dict_input:
            for movement in dict_input[frame]:
                current_state[movement['lane']] += 1
        
        lst = current_state.copy()
        state_dict.append(current_state)

class Model(nn.Module):
    """Neural network model for traffic light control."""
        ...
        for junction in all_junctions:
            if step > 150:
                if traffic_lights_time[junction] <= 0:
                    # Calculate vehicle changes since last step
                    vehicles_per_lane = [a_i - b_i for a_i, b_i in zip(state_dict[step], state_dict[last_step])]
                    
                    # Log traffic state with detailed movement information
                    ...
                    # Log traffic light changes
                    logger.info("Traffic Light Change:")
                    ...
                    # Log status updates every second
                    if traffic_lights_time[junction] % 1 < 0.07:
